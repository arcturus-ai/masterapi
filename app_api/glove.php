	...
	if(($username== "TaTaStEEl") && ($password == "TaTaStEEl@24052022" )){
		...
		if ($project == 'Admin'){
			...
			if($site == 'TND' or $site == 'Utility' or $site == 'RMHS' or $site == 'Sinter Plant' or $site == 'RMBB' or $site == 'Coke Plant' or $site == 'BPP' or $site == 'SMS' or $site == 'Blast Furnace' or $site == 'LCP' or $site == 'HSM' or $site == 'CRM' or $site == 'Pellet Plant'){
			    $sql="SELECT sum(glove) as glove FROM ai_safety Where Department = '$site'";
				...
				$success = 1;
				$sql="SELECT * FROM ai_safety Where Department  = '$site' and glove > 0 ORDER BY time Desc LIMIT {$begin},{$row_page}";
				$result=mysqli_query($con,$sql);
				$glove=array();
				while($row=mysqli_fetch_assoc($result)){
				$glove[]=$row;
			    ...
			    $sql="select sum(events = 0) as 'close',sum(events = 1) as 'open' FROM ai_safety where Department  = '$site' and glove ORDER BY time Desc LIMIT {$begin},{$row_page}";
				$result=mysqli_query($con,$sql);
				$open_close=array();
				while($row=mysqli_fetch_assoc($result)){
				$open_close[]=$row;
		    ...
		    $sql="SELECT sum(glove>0) as glove FROM ai_safety Where Department = '$project'";
    		$result=mysqli_query($con,$sql);
    		$glove_tot=array();
    		while($row=mysqli_fetch_assoc($result)){
    		$glove_tot[]=$row;
		    ...
		    $massege = "Successfully Data fetch !";
		    $success = 1;
			$sql="SELECT * FROM ai_safety Where Department  = '$project' and glove > 0 ORDER BY time Desc LIMIT {$begin},{$row_page}";
			$result=mysqli_query($con,$sql);
			$glove=array();
			while($row=mysqli_fetch_assoc($result)){
			$glove[]=$row;
			...
			$sql="select sum(events = 0) as 'close',sum(events = 1) as 'open' FROM ai_safety where Department  = '$project' and glove ORDER BY time Desc LIMIT {$begin},{$row_page}";
			$result=mysqli_query($con,$sql);
			$open_close=array();
			while($row=mysqli_fetch_assoc($result)){
			$open_close[]=$row;
