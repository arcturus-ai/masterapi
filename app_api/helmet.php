	...
	if(($username== "TaTaStEEl") && ($password == "TaTaStEEl@24052022" )){
		...
		if ($project == 'Admin'){
			...
			if($site == 'TND' or $site == 'Utility' or $site == 'RMHS' or $site == 'Sinter Plant' or $site == 'RMBB' or $site == 'Coke Plant' or $site == 'BPP' or $site == 'SMS' or $site == 'Blast Furnace' or $site == 'LCP' or $site == 'HSM' or $site == 'CRM' or $site == 'Pellet Plant'){
			    $sql="SELECT sum(helmet) as helmet FROM ai_safety Where Department = '$site'";
        		$result=mysqli_query($con,$sql);
        		$helmet_tot=array();
        		while($row=mysqli_fetch_assoc($result)){
        		$helmet_tot[]=$row;
        		}
        		$sql="SELECT * FROM ai_safety Where Department = '$site' and helmet";
        		$result=mysqli_query($con,$sql);
        		$totalItems=array();
        		while($row=mysqli_fetch_assoc($result)){
        		$totalItems[]=$row;
				...
				$success = 1;
				$sql="SELECT * FROM ai_safety Where Department  = '$site' and helmet > 0 ORDER BY time Desc LIMIT {$begin},{$row_page}";
			    ...
			    $sql="select sum(events = 0) as 'close',sum(events = 1) as 'open' FROM ai_safety where Department  = '$site' and helmet ORDER BY time Desc LIMIT {$begin},{$row_page}";
				$result=mysqli_query($con,$sql);
				$open_close=array();
				while($row=mysqli_fetch_assoc($result)){
				$open_close[]=$row;
		    ...
		    $sql="SELECT sum(helmet>0) as helmet FROM ai_safety Where Department = '$project'";
    		$result=mysqli_query($con,$sql);
    		$helmet_tot=array();
    		while($row=mysqli_fetch_assoc($result)){
    		$helmet_tot[]=$row;
    		...
    		$sql="SELECT * FROM ai_safety Where Department = '$project' and helmet";
    		$result=mysqli_query($con,$sql);
    		$data=array();
    		while($row=mysqli_fetch_assoc($result)){
    		$totalItems[]=$row;
		    ...
		    $massege = "Successfully Data fetch !";
		    $success = 1;
			$sql="SELECT * FROM ai_safety Where Department  = '$project' and helmet > 0 ORDER BY time Desc LIMIT {$begin},{$row_page}";
			$result=mysqli_query($con,$sql);
			$helmet=array();
			while($row=mysqli_fetch_assoc($result)){
			$helmet[]=$row;
			...
			$sql="select sum(events = 0) as 'close',sum(events = 1) as 'open' FROM ai_safety where Department  = '$project' and helmet ORDER BY time Desc LIMIT {$begin},{$row_page}";
			$result=mysqli_query($con,$sql);
			$data=array();
			while($row=mysqli_fetch_assoc($result)){
			$open_close[]=$row;
