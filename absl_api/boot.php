	...
	if(($username== "l&tmhi") && ($password == "L&tP0wermHi@03092021" )){
		...
		if ($project == 'HO'){
			...
			if($site == 'Ghatampur' or $site == 'Khurja' or $site == 'Buxar'){
			    $sql="SELECT sum(boot) as boot FROM ai_safety Where Project = '$site'";
				...
				$success = 1;
				$sql="SELECT * FROM ai_safety Where Project  = '$site' and boot > 0 ORDER BY time Desc LIMIT {$begin},{$row_page}";
				$result=mysqli_query($con,$sql);
				$boot=array();
				while($row=mysqli_fetch_assoc($result)){
				$boot[]=$row;
			    ...
			    $sql="select sum(events = 0) as 'close',sum(events = 1) as 'open' FROM ai_safety where Project  = '$site' and boot ORDER BY time Desc LIMIT {$begin},{$row_page}";
				$result=mysqli_query($con,$sql);
				$open_close=array();
				while($row=mysqli_fetch_assoc($result)){
				$open_close[]=$row;
		    ...
		    $sql="SELECT sum(boot>0) as boot FROM ai_safety Where Project = '$project'";
    		$result=mysqli_query($con,$sql);
    		$boot_tot=array();
    		while($row=mysqli_fetch_assoc($result)){
    		$boot_tot[]=$row;
		    ...
		    $massege = "Successfully Data fetch !";
		    $success = 1;
			$sql="SELECT * FROM ai_safety Where Project  = '$project' and boot > 0 ORDER BY time Desc LIMIT {$begin},{$row_page}";
			$result=mysqli_query($con,$sql);
			$boot=array();
			while($row=mysqli_fetch_assoc($result)){
			$boot[]=$row;
			...
			$sql="select sum(events = 0) as 'close',sum(events = 1) as 'open' FROM ai_safety where Project  = '$project' and boot ORDER BY time Desc LIMIT {$begin},{$row_page}";
			$result=mysqli_query($con,$sql);
			$open_close=array();
			while($row=mysqli_fetch_assoc($result)){
			$open_close[]=$row;
